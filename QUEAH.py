...
class Application:
    def __init__(self, master=None):
        ...
        piecesLeft1 = Label(master, text="Peças faltantes: 6", font=MEDIUMFONT)
        piecesLeft1.place(relx=0.07, rely=0.5, anchor=CENTER)

        piecesLeft2 = Label(master, text="Peças faltantes: 6", font=MEDIUMFONT)
        piecesLeft2.place(relx=0.93, rely=0.5, anchor=CENTER)

        restart = Button(master, text="Reiniciar partida", font=MEDIUMFONT, command=self.reiniciarPartida)
            ...
            for j in range(5):
                if MATRIZ_VISUALIZACAO[i][j] != X:
                    if [i, j] in POSICOES_INICIAIS_AZUL:
                        ...
                        peca.bind("<Button-1>", lambda e: self.casaClicada())
                        peca.image = self.pinoAzul
                        MATRIZ_VISUALIZACAO[i][j] = 1
